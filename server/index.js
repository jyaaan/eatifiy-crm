        ...
        .then(medias => {
          ...
          async.mapSeries(medias, (media, next) => {
            ...
            async.mapSeries(dedupedPublicLikers, (liker, next) => {
              if (liker != 'barbiebuli' && liker != 'mlb_hunchoo') {
                scrapeSave(liker)
                  .then(user => {
              ...
              database.getInfluencers(publicLikerIds)
                .then(influencers => {
                  ...
                  var influencerData = influencers.map(influencer => {
                    ...
                    influencer.recent_post_count + ',' + influencer.recent_like_count + ',' + influencer.recent_comment.count + ',' + ((influencer.recent_post_duration / 3600) / influencer.recent_post_count) + ',' +
                      ...
                      database.getConsumers(publicLikerIds)
                        .then(consumers => {
                          var consumerData = consumers.map(consumer => {
                            ...
                            consumer.recent_post_count + ',' + consumer.recent_like_count + ',' + consumer.recent_comment.count + ',' + ((consumer.recent_post_duration / 3600) / consumer.recent_post_count) + ',' +
